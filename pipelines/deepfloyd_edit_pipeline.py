...
class AttnAddedKVProcessor_Mine:
    r"""
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if not attn.only_cross_attention:       # self+ cross
        ...
        ### NOTE: save the attention probabilities  attn.only_cross_attention = is_cross
        # attention_probs = self.attnstore(attention_probs, True, self.place_in_unet, self.cross_replace_steps, self.self_replace_steps,
        #                     between_step=True)
        if attn.only_cross_attention:
            attention_probs = self.attnstore(attention_probs, True, self.place_in_unet, 
                                             self.cross_replace_steps, self.self_replace_steps,
                                                between_step=True)
        else:
            attention_probs_self = self.attnstore(attention_probs[:,:, MAX_LEN:], False, self.place_in_unet, 
                                                  self.cross_replace_steps, self.self_replace_steps,
                                                    between_step=False)
            attention_probs_cross = self.attnstore(attention_probs[:,:, :MAX_LEN], True, self.place_in_unet, 
                                                   self.cross_replace_steps, self.self_replace_steps,
                                                    between_step=True)
            
            attention_probs = torch.cat([attention_probs_cross,attention_probs_self], dim=-1)
            
        hidden_states = torch.bmm(attention_probs, value)
        hidden_states = attn.batch_to_head_dim(hidden_states)       # input: 24 1024 64 
...
class AttentionStore:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str, 
                        cross_replace_steps, self_replace_steps, between_step=False):
...
class AttentionReplace(AttentionStore):
    def __call__(self, attn, is_cross: bool, place_in_unet: str, 
                        cross_replace_steps=50, self_replace_steps=50,
                        between_step=False):
...
class IFEditPipeline(DiffusionPipeline):
    ...
    def register_attention_control(self):
        ...
        for name in self.unet.attn_processors.keys():
            ...
            cross_att_count += 1
            attn_procs[name] = AttnAddedKVProcessor_Mine(
    ...
    def __call__(
        ...
        ### p2p editing parameters
        cross_replace_steps=40, 
        self_replace_steps=40,
        local=False,
        indices_local=None,
        ...
        ### NOTE; initialize some editing related parameters
        self.cross_replace_steps = cross_replace_steps
        self.self_replace_steps = self_replace_steps
        ...
        BATCHSIZE = 2
        intermediate_images=torch.cat([intermediate_images] * BATCHSIZE) 

        # init_noise=intermediate_images
        ...
        ### NOTE: register the attention store
        if replace:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ### NOTE: split the conditional and unconditional embeddings
                if do_classifier_free_guidance:
                    # _, cond_embeddings = prompt_embeds.chunk(2)
                    
                    uncond_embeddings = uncond_embeddings_list[i].to(device)
                    prompt_embeds_t=torch.cat([uncond_embeddings,
                                             uncond_embeddings, 
                                             cond_embeddings,
                                             edit_prompt_embeds])
                ...
                ### NOTE: save the ddim inversion traces
                # inter_img_list.append(intermediate_images.cpu().detach())
                if local:
                    x_t = intermediate_images
                    ### NOTE: x_t[0]=x_t[0]+mask*(x_t[0]-x_t[0]); x_t[1]=x_t[0]+mask*(x_t[1]-x_t[0])
                    x_t = x_t[:1] + mask * (x_t - x_t[:1])
                    intermediate_images = x_t
                ...
                ### NOTE: save intermediate results

        image, edit_image = intermediate_images[0].unsqueeze(0), intermediate_images[1].unsqueeze(0)
        ...
        return IFPipelineOutput(images=image, nsfw_detected=nsfw_detected, watermark_detected=watermark_detected), edit_image, inter_img_list
