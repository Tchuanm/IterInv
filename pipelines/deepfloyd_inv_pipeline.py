...
class IFInvPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        ### NOTE: save the unconditional embeddings
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    latent_prev = latent_prev.to(device)

                    if not is_NPI:     
                        uncond_embeddings = uncond_embeddings.detach().clone().requires_grad_(True)
                    else:
                        # uncond_embeddings = cond_embeddings.detach().clone().requires_grad_(True)
                         # negative prompt inversion;   uncond_embeddings <==  cond_embedding.  fasten but little worse
                        uncond_embeddings = cond_embeddings.detach().clone().requires_grad_(False)

                    cond_embeddings = cond_embeddings.detach().clone().requires_grad_(False)
                    
                    ### NOTE: if it is NPI, we don't need any optimization since it is equal to DDIM 
                    if not is_NPI:
                        opt = torch.optim.Adam([uncond_embeddings], lr= learning_rate * (1. - i / 100.))
                        ...
                        with torch.enable_grad():
                            for j in range(null_inner_steps):
                                ...
                                noise_pred = self.unet(model_input,
                                                    t,
                                                    encoder_hidden_states=context,
                                                    cross_attention_kwargs=cross_attention_kwargs,
                                                    ).sample
                                ### NOTE: consider modify the above for loss scaler
                                ### NOTE: CFG: classifier free guidance.
                                # if do_classifier_free_guidance:
                                if True:
                                    noise_pred_uncond_loop, noise_pred_text_loop = noise_pred.chunk(2)
                                    noise_pred_uncond_loop, _ = noise_pred_uncond_loop.split(model_input.shape[1], dim=1)
                                    noise_pred_text_loop, predicted_variance_loop = noise_pred_text_loop.split(model_input.shape[1], dim=1)
                                    noise_pred_loop = noise_pred_uncond_loop + guidance_scale * (noise_pred_text_loop - noise_pred_uncond_loop)
                                    noise_pred_loop = torch.cat([noise_pred_loop, predicted_variance_loop], dim=1)

                                # noise_pred_uncond_text, noise_pred_cond_text = noise_pred.chunk(2)
                                # noise_pred_loop = noise_pred_uncond_text + guidance_scale * (noise_pred_cond_text - noise_pred_uncond_text)
                                
                                latents_prev_rec = self.scheduler.step(noise_pred_loop, t, intermediate_images, **extra_step_kwargs).prev_sample
                                loss = ((latents_prev_rec-latent_prev)**2).sum()

                                loss.backward(retain_graph=False)
                                opt.step()
                                opt.zero_grad()
                                # uncond_embeddings = uncond_embeddings - uncond_embeddings.grad*step_size[num_inference_steps-1-i]
        ...
        return IFPipelineOutput(images=image, nsfw_detected=nsfw_detected, watermark_detected=watermark_detected), \
                    inter_img_list, uncond_embeddings_list
