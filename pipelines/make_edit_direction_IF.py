...
from deepfloyd_edit_pix2pix_zero_pipeline import IFEditZeroPipeline
...
if __name__=="__main__":
    ...
    parser.add_argument('--file_source_sentences', default='assets/cat_1.txt')
    parser.add_argument('--file_target_sentences', default='assets/dog_1.txt')
    parser.add_argument('--output_folder', default='assets/')
    parser.add_argument('--model_path', type=str, default='DeepFloyd/IF-I-M-v1.0')
    args = parser.parse_args()
    
    # load the model
    if torch.cuda.is_available():
        pipe = IFEditZeroPipeline.from_pretrained(args.model_path, torch_dtype=torch.float16).to(device)
    else:
        pipe = IFEditZeroPipeline.from_pretrained(args.model_path).to(device)
...
"""
  python src/make_edit_direction_IF.py \
    --file_source_sentences assets/cat.txt \
    --file_target_sentences assets/dog.txt \
