...
class StableDiffusionUpscaleInvPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        ### NOTE: 5.xx set auxilary timesteps
        # aux_scheduler.set_timesteps(num_inference_steps, device=device)
        # aux_timesteps = aux_scheduler.timesteps
        ...
        ### NOTE: latent list, z0, z1, ... , zT
        init_latent = latents
        latent_expect = latents.detach()

        latents_list=[]
        if return_latents_list:
            latents_list.append(latent_expect)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in reversed(list(enumerate(timesteps))):
                ...
                ### NOTE: update latent with optimizer
                ### NOTE: set optimizer
                # latents = latents.clone().detach().requires_grad_(True)
                # concat_optim = torch.optim.AdamW([latents], lr= 1e-2 * (1. - i / 100.))

                for j_loop in range(inner_steps):
                    with torch.enable_grad():
                        latents = latents.clone().detach().requires_grad_(True)

                        # NOTE:expand the latents if we are doing classifier free guidance
                        ...
                        latents_pred = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                        ...
                        # concat_optim.step()
                        # concat_optim.zero_grad()
                        latents = latents - latents.grad*step_size[num_inference_steps-1-i]

                ### NOTE: append to the list
                latent_expect = latents.detach()
                if return_latents_list:
                    latents_list.append(latent_expect.cpu())
        ...
        if not output_type == "latent":
            ...
            ### NOTE: if we want to run safety_checker, we need to transform the image type and value range
