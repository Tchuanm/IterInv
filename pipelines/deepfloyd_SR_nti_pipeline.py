    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in reversed(list(enumerate(timesteps))):
                ...
                # opt = torch.optim.Adam([uncond_embeddings], lr= learning_rate * (1. - i / float(num_inference_steps*2.0)))
                ...
                for j_loop in range(inner_steps):
                    with torch.enable_grad():
                        ...
                        # opt.step()
                        # opt.zero_grad()
                        # if j_loop > 100 and (loss <= 1.0):
                        #     break
                        ### NOTE: option1: optimizer 

                        ###NOTE: from Attend and Excite
                        ### NOTE: option2 
                        uncond_embeddings.data = uncond_embeddings.data - uncond_embeddings.grad*step_size[num_inference_steps-1-i]
