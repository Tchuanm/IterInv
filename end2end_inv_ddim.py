...
def arguments():
    ...
    parser.add_argument('--no-is_NPI', dest='is_NPI', action='store_false')
...
if __name__=="__main__":
    ...
    if  args.enable_2:
        # stage_2 = IFSuperResolutionInvPipeline.from_pretrained(
    ...
    if args.enable_2:
        ...
        # ## NOTE: ddim trace
        generator = torch.manual_seed(0)
        stage_2_rec.scheduler = DDIMInverseScheduler.from_config(stage_2_rec.scheduler.config)
        ddim_2_inv_tuple, _ = stage_2_rec(
                image=image_rec_1, 
                generator=generator,
                prompt_embeds=prompt_embeds, 
                negative_prompt_embeds=negative_embeds, 
                ...
                image_init=inv_raw_image_1,
                )
        ddim_2_inv = ddim_2_inv_tuple.images

        generator = torch.manual_seed(0)
        stage_2_rec.scheduler = DDIMScheduler.from_config(stage_2_rec.scheduler.config)
        ddim_2_rec_tuple , ddim_inv_2_list = stage_2_rec(
                image=image_rec_1, 
                generator=generator,
                prompt_embeds=prompt_embeds, 
                negative_prompt_embeds=negative_embeds, 
                ...
                noise_level=noise_level_2,
                guidance_scale=1.0,
                image_init=ddim_2_inv,
                )

        image_2_rec_ddim=ddim_2_rec_tuple.images
        torch.save(image_2_rec_ddim.detach().to('cpu'), f"{saving_path}/image_2_rec_ddim.pt")
        pt_to_pil(image_2_rec_ddim)[0].save(f"images/if_stage_II_rec_ddim.png")
    ...
    if args.enable_3:
        image_2_rec_ddim = torch.load(f"{saving_path}/image_2_rec_ddim.pt")
