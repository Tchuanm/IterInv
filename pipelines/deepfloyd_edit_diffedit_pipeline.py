...
import torch
import os
from pipelines.cross_attention import prep_unet
import warnings
...
class IFDiffEditPipeline(DiffusionPipeline):
    ...
    def generate_mask(
        ...
        mask_encode_strength: Optional[float] = 0.4,
        mask_thresholding_ratio: Optional[float] = 2.5,
        ...
        image = image.repeat_interleave(num_maps_per_mask, dim=0).to(device=device, dtype=self.unet.dtype)
        noise = randn_tensor(image.shape, generator=generator, device=device, dtype=self.unet.dtype)
        image = self.scheduler.add_noise(image, noise, encode_timestep)

        model_input = torch.cat([image] * (4 if do_classifier_free_guidance else 2))
        model_input = self.scheduler.scale_model_input(model_input, encode_timestep)
        ...
        if do_classifier_free_guidance:
            noise_pred_neg_src, noise_pred_source, noise_pred_uncond, noise_pred_target = noise_pred.chunk(4)
            noise_pred_neg_src, _ = noise_pred_neg_src.split(model_input.shape[1], dim=1)
            noise_pred_source, _ = noise_pred_source.split(model_input.shape[1], dim=1)
            noise_pred_source = noise_pred_neg_src + guidance_scale * (noise_pred_source - noise_pred_neg_src)

            noise_pred_uncond, _ = noise_pred_uncond.split(model_input.shape[1], dim=1)
            noise_pred_target, _ = noise_pred_target.split(model_input.shape[1], dim=1)
            noise_pred_target = noise_pred_uncond + guidance_scale * (noise_pred_target - noise_pred_uncond)
        else:
            noise_pred_source, noise_pred_target = noise_pred.chunk(2)
    ...
    def invert(
        ...
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        image_init: Union[PIL.Image.Image, np.ndarray, torch.FloatTensor] = None, 
        ...
        ### NOTE: change this part to modify the intermediate_images compatible with real images
        if image_init is None:
            ...
            if len(image_init.shape) == 3:
                intermediate_images = torch.stack([image_init],dim=0).type(prompt_embeds.dtype).to(device)
            else:
                intermediate_images = image_init.type(prompt_embeds.dtype).to(device)
        

        # 7.  Denoising loop
        num_warmup_steps = len(timesteps) - num_inference_steps * self.inverse_scheduler.order
        inverted_intermediate_images = []

        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # expand the intermediate_images if we are doing classifier free guidance
                    ...
                    for _ in range(num_reg_steps):
                        if lambda_auto_corr > 0:
                            for _ in range(num_auto_corr_rolls):
                                ...
                                var_epsilon = self.get_epsilon(var, model_input.detach(), t)
                ...
                intermediate_images = self.inverse_scheduler.step(noise_pred, t, intermediate_images).prev_sample
                inverted_intermediate_images.append(intermediate_images.detach().clone())
        ...
        assert len(inverted_intermediate_images) == len(timesteps)
        intermediate_images = torch.stack(list(reversed(inverted_intermediate_images)), 1)

        # 8. Post-processing
        image = intermediate_images.detach()

        return IFPipelineOutput(images=image, nsfw_detected=None, watermark_detected=None)
    ...
    def __call__(
        ...
        callback_steps: int = 1,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        image_invs: Union[PIL.Image.Image, np.ndarray, torch.FloatTensor] = None, 
        ...
        # 6. Preprocess image inves.
        if isinstance(image_invs, list) and any(isinstance(l, torch.Tensor) and l.ndim == 5 for l in image_invs):
            image_invs = torch.cat(image_invs).detach()
        elif isinstance(image_invs, torch.Tensor) and image_invs.ndim == 5:
            image_invs = image_invs.detach()
        else:
            image_invs = self.image_processor.preprocess(image_invs).detach()

        image_shape = (self.unet.config.in_channels, height, width)
        if image_invs.shape[-3:] != image_shape:
            raise ValueError(
                f"Each image in `image_invs` must have shape {image_shape}, "
                f"but has shape {image_invs.shape[-3:]}"
            )
        if image_invs.ndim == 4:
            image_invs = image_invs.reshape(batch_size, len(timesteps), *image_shape)
        if image_invs.shape[:2] != (batch_size, len(timesteps)):
            raise ValueError(
                f"`image_invs` must have batch size {batch_size} with images from {len(timesteps)}"
                f" timesteps, but has batch size {image_invs.shape[0]} with images from"
                f" {image_invs.shape[1]} timesteps."
            )
        
        image_invs = image_invs.transpose(0, 1).repeat_interleave(num_images_per_prompt, dim=1)
        image_invs = image_invs.to(device=device, dtype=prompt_embeds.dtype)

        
        # NOTE: save some of them
        # 7. Prepare extra step kwargs. TODO: Logic should ideally just be moved out of the pipeline
        extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)

        # 7.  Denoising loop
        intermediate_images = image_invs[0].clone()
        ...
        with torch.no_grad():
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    # compute the previous noisy sample x_t -> x_t-1
                    intermediate_images = self.scheduler.step(noise_pred, t, intermediate_images, **extra_step_kwargs).prev_sample

                    # mask with inverted latents from appropriate timestep - use original image latent for last step
                    intermediate_images = intermediate_images * mask_image + image_invs[i] * (1 - mask_image)
        ...
        # 8. Post-processing
        edit_image = intermediate_images.detach()

        if output_type == "pil":
            # 8. Post-processing
            image_rec = (image_rec / 2 + 0.5).clamp(0, 1)
            image_rec = image_rec.cpu().permute(0, 2, 3, 1).float().numpy()

            # 9. Run safety checker
            image_rec, nsfw_detected, watermark_detected = self.run_safety_checker(image_rec, device, prompt_embeds.dtype)
            ...
            if self.watermarker is not None:
                self.watermarker.apply_watermark(image_rec, self.unet.config.sample_size)
        elif output_type == "pt":
            nsfw_detected = None
            watermark_detected = None
            image_rec = None
        ...
        if not return_dict:
            return (image_rec, nsfw_detected, watermark_detected)

        return IFPipelineOutput(images=image_rec, nsfw_detected=nsfw_detected, watermark_detected=watermark_detected), edit_image
