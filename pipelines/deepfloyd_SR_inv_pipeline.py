...
class IFSuperResolutionInvPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def __call__(
        ...
        ### NOTE: save some other info
        # upscale_list=[]
        # upscale_list.append(image)
        # upscale_list.append(upscaled)

        noise_level = torch.tensor([noise_level] * upscaled.shape[0], device=upscaled.device)
        noise = randn_tensor(upscaled.shape, generator=generator, device=upscaled.device, dtype=upscaled.dtype)
        upscaled = self.image_noising_scheduler.add_noise(upscaled, noise, timesteps=noise_level)

        # upscale_list.append(noise)
        # upscale_list.append(upscaled)
        ...
        ### NOTE: save the intermediate steps 
        #####   ====================================================================
        inter_img_list=[]
        inter_img_list.append(intermediate_images.detach().cpu())
        latent_expect = intermediate_images.detach().clone()
        ...
        ### NOTE: option2 
        ########### ====================================================================
        # init_noise=intermediate_images.detach().clone()

        # 8. Denoising loop
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            # for i, t in enumerate(timesteps):
            for i, t in reversed(list(enumerate(timesteps))):
                ### NOTE: option1: optimizer 
                # intermediate_images = ddim_inv_list[i].cuda().clone().detach().requires_grad_(True)
                intermediate_images = intermediate_images.clone().detach().requires_grad_(True)
                
                # noise_level_loop = torch.tensor([20] * intermediate_images.shape[0], device=intermediate_images.device)
                # noise_loop = randn_tensor(intermediate_images.shape, generator=generator, device=intermediate_images.device, dtype=intermediate_images.dtype)
                # intermediate_images = self.image_noising_scheduler.add_noise(intermediate_images, noise_loop, timesteps=noise_level_loop)

                # opt = torch.optim.SGD([intermediate_images], lr= learning_rate * (1. - i / 100.), momentum=0.9, weight_decay=5e-4)
                # opt = torch.optim.Adam([intermediate_images], lr= learning_rate * (1. - i / 100.))
                # opt = torch.optim.AdamW([intermediate_images], lr= learning_rate * (1. - i / 100.))
                ### NOTE: option1: optimizer 
                
                # loss_opt=1000.0
                # loss_prime=1000.0
                # step_size_opt=step_size

                for j_loop in range(inner_steps):
                    with torch.enable_grad():

                        ### NOTE: option2: update latent
                        intermediate_images = intermediate_images.clone().detach().requires_grad_(True)
                        ### NOTE: option2: update latent
                        ...
                        ### NOTE: actually the above line is doing nothing
                        ...
                        latents_pred = self.scheduler.step(
                            noise_pred, t, intermediate_images, **extra_step_kwargs, return_dict=False
                        )[0]
                        
                        ### NOTE: compute loss
                        loss = ((latents_pred-latent_expect)**2).sum()
                        
                        if j_loop % 10 == 0:
                            print_string = f"Step {num_inference_steps-1-i}, Attend {j_loop} | Loss: {loss.item():0.6f}"
                            print(print_string)  

                        loss.backward()

                        ###NOTE: from Null-Text inversion
                        ### NOTE: option1: optimizer 
                        # opt.step()
                        # opt.zero_grad()
                        ### NOTE: option1: optimizer 

                        ###NOTE: from Attend and Excite
                        ### NOTE: option2 
                        intermediate_images = intermediate_images - intermediate_images.grad*step_size[num_inference_steps-1-i]
                        # intermediate_images = intermediate_images - intermediate_images.grad*step_size_opt[num_inference_steps-1-i]
                        ### NOTE: option2                         

                ### NOTE: append to the list
                latent_expect = intermediate_images.detach().clone()
                inter_img_list.append(latent_expect.cpu())
                ...
                ### NOTE: save the intermediate images
                # inter_img_list.append(intermediate_images)
        ...
        return IFPipelineOutput(images=image, nsfw_detected=nsfw_detected, watermark_detected=watermark_detected), inter_img_list