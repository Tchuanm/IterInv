...
class AttnAddedKVProcessor_Mine:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if not attn.only_cross_attention:
            ...
            _key = attn.head_to_batch_dim(key)
            _value = attn.head_to_batch_dim(value)
        ...
        if attn.only_cross_attention:
            self.attnstore(attention_probs, True, self.place_in_unet, 
                            hidden_states, query, encoder_hidden_states_key_proj, encoder_hidden_states_value_proj,
                            between_step=True)
        else:
            self.attnstore(attention_probs[:,:, 77:], False, self.place_in_unet, 
                            hidden_states, query, _key, _value,
                            between_step=False)
            self.attnstore(attention_probs[:,:, :77], True, self.place_in_unet, 
                            hidden_states, query, encoder_hidden_states_key_proj, encoder_hidden_states_value_proj,
                            between_step=True)
...
class AttentionStore:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str, 
                        hidden_states, query, key, value, between_step=False):
...
class IFStudyPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        return IFPipelineOutput(images=image, nsfw_detected=nsfw_detected, watermark_detected=watermark_detected), self_avg_dict, cross_avg_dict
