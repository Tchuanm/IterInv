...
def save_crossattn(input_path, caption, inv_cross_avg_dict, denoise_cross_avg_dict, results_folder, RES=16):
    ...
    if denoise_cross_avg_dict is not None:
    ...
    attn_img1_noimg, _ = show_cross_attention(prompts, inv_crossattn)
    if denoise_cross_avg_dict is not None:
        attn_img2, _ = show_cross_attention_plus_orig_img(prompts, denoise_crossattn, orig_image=org_image)

    attn_img1.save(os.path.join(results_folder,f'crossattn_inv_{RES}.png')) 
    attn_img1_noimg.save(os.path.join(results_folder,f'crossattn_inv_{RES}_noimg.png')) 
    if denoise_cross_avg_dict is not None:
        attn_img2.save(os.path.join(results_folder,f'crossattn_denoise_{RES}.png')) 